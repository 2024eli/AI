...
def scoreCalc(pzl, token, posMoves):
  ...
  if token.lower() == 'o': opptoken = "x"
  #mobility
  #CONSTANT is so that the mobility is weighted less in later games
  # if (count:=pzl.count(".")) < 20:
  #   CONSTANT = count-2
  # else:
  #   CONSTANT = 20
  score = 0
  # for mv in posMoves:
  #   oppMoves = findMoves(makeMoves(pzl, token, mv), opptoken)
  #   if len(oppMoves) < oppMoveLen:
  oppMoveLen = len(findMoves(pzl, opptoken))
  score = 3 * (-oppMoveLen)
  # score = CONSTANT*(len(posMoves))
  cornerCount = 0
  for i in corner_points:
    if pzl[i] == token: cornerCount+=1
    elif pzl[i] == opptoken: cornerCount-=1
  score += 100*cornerCount
  #score += stability(pzl, token, posMoves)*200
  return score
