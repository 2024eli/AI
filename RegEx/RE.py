...
myRegexLst = [
  ...
  r"/^((?!1[01]1)[10])*$/",  # 59 ----
  r"/(?!.*010)^[01]*$/",  # 60 16 14
  r"/(?!.*010|101)^[01]*$/",  # 61 20 20
  r"/^(1|0)([01]*\1)*$/",  # 62 17 14
  r"/\b(?!(\w)*\w*\1\b)\w+/i",  # 63 21 21
  r"/\b(?=(\w)*(\w)*\w*(\1(\w)*\4|\2\w*\1|\1\w*\2))\w*/i",  # 64 49 43
  r"/\b(?=(\w)*(\w*\1){2,})(?!\w*(?!\1)(\w)\w*\3)\w*/i",  # 65 47 44
  r"/\b(?!\w*([aeiou])\w*\1)\w*(\w*[aeiou]){5}\w*/i",  # 66 66 39
  r"/^(?=((10*1)?0*)*$)([10]{2})*[10]$/",  # 67 36 22
  r"/^(0|(1(01*0)*10*)+)$/",  # 68 20 19
  r"/^1((10*1)*(01*0)?)*(01*)?$/",  # 69 26 19
  r"/^[a-z]*(([aeiou])(?!.*\2)\w*){5}\w*$/m", #70 36
  r"/^([^aeiou\WA-Z]*[aeiou]){5}[^aeiou\W]*$/m", #71 39
  r"/^[a-z]*[^aeiou\WA-Z]w(?!r?h?[aeiou])\w*$/m", #72 40
  r"/(?![A-Z])^((?=(.)(.)(\w)).*\4\3\2|aa?)$/m", #73 39
  r"/^[a-z]*(bt|tb)[^bt\W]*$/m", #74 28
  r"/^([a-z])*\1\w*$/m", #75 15
  r"/(\w)*(\w*\1){5}\w*$/m", #76 19
  r"/((\w)\2){3}\w*$/m", #77 15
  r"/(\w*[^aeiou\W]){13}\w*/m", #78 22
  r"/^(?!(.)*(.*\1){2})[a-z]+$/m", #79 25
]
